...
def home():
    ...
    if 'sendUserPass' in request.form.keys():
        try:
            ...
            # to pass back defined values present during the post - thereby
            # keeping permanence on the page
        ...
        try:
            ...
            somedict = {'public_key':
                        request.form['public_key'], 'message': request.form['message']}
...
def convert_to_wif():
    ...
    # XXX: Untested
    # http://gobittest.appspot.com/PrivateKey and
...
def get_keypair(priv_key):
    ...
    keypair = {'public_key': generate_keypair.generate_btc_address(
        int(priv_key, 16))[3], 'private_key': str(priv_key)}
...
def encrypt_message(somedict):
    ...
    # XXX:WIP
    if somedict['message']:
        somedict['message'] = 'pretend this message is encypted now'
    return somedict
